...
import { BarChartSquare02, CursorClick01, Globe02, Palette, PlayCircle, SearchLg, Zap } from "@untitledui/icons";
...
const HeroSection = () => {
    return (
        <div className="relative overflow-hidden bg-brand-section">
            <img alt="Grid of dots" aria-hidden="true" loading="lazy" src="https://www.untitledui.com/patterns/light/grid-dot-sm-desktop.svg" className="pointer-events-none absolute top-0 left-1/2 z-0 hidden max-w-none -translate-x-1/2 opacity-20 md:block" />
            <img alt="Grid of dots" aria-hidden="true" loading="lazy" src="https://www.untitledui.com/patterns/light/grid-dot-sm-mobile.svg" className="pointer-events-none absolute top-0 left-1/2 z-0 max-w-none -translate-x-1/2 opacity-20 md:hidden" />
            ...
            <section className="relative py-16 md:py-24">
                <div className="mx-auto w-full max-w-container px-4 md:px-8">
                    <div className="flex max-w-5xl flex-col">
                        ...
                        <p className="mt-4 max-w-(--breakpoint-sm) text-lg text-tertiary_on-brand md:mt-6 md:text-xl">
                            — Professional dealership websites with inventory integration, lead capture, SEO optimization, and real-time analytics. Launch your site in minutes, not months.
                        </p>
                ...
                <div className="mx-auto mt-16 w-full max-w-container px-4 md:px-8">
                    <img alt="VinTraxx Websites Dashboard" src={webLandingImage.src} className="md:[360px] h-60 w-full rounded-lg object-cover lg:h-129" />
                </div>
            </section>
        </div>
    );
            ...
            <div className="mx-auto max-w-container px-4 md:px-8">
                <div className="flex flex-col gap-8">
                    ...
                    <div className="flex flex-wrap justify-center gap-x-8 gap-y-4 xl:gap-x-6">
                        <div className="flex h-9 items-center gap-2 text-lg font-semibold text-primary_on-brand opacity-85 md:h-12 md:text-xl">ABC Motors</div>
                        <div className="flex h-9 items-center gap-2 text-lg font-semibold text-primary_on-brand opacity-85 md:h-12 md:text-xl">Elite Auto</div>
                        <div className="flex h-9 items-center gap-2 text-lg font-semibold text-primary_on-brand opacity-85 md:h-12 md:text-xl">Premier Cars</div>
                        <div className="flex h-9 items-center gap-2 text-lg font-semibold text-primary_on-brand opacity-85 md:h-12 md:text-xl">Valley Auto Group</div>
                        <div className="flex h-9 items-center gap-2 text-lg font-semibold text-primary_on-brand opacity-85 md:h-12 md:text-xl">City Wheels</div>
                    </div>
                </div>
            </div>
        </section>
    ...
    return (
        <section className="bg-primary">
            <div className="bg-secondary pt-16 pb-[112px] md:pt-24 md:pb-40">
                <div className="mx-auto grid w-full max-w-container grid-cols-1 gap-12 px-4 md:gap-16 md:px-8 lg:grid-cols-2 lg:gap-24">
                    ...
                    <ul className="grid grid-cols-1 gap-10 md:grid-cols-2 md:gap-12 lg:grid-cols-1">
                        {[
                            { title: "Professional Templates", subtitle: "Beautiful, mobile-responsive templates designed for automotive dealers. Customize colors, fonts, and layouts to match your brand.", icon: Palette },
                            { title: "Inventory Integration", subtitle: "Real-time sync with VinLane IMS. Your inventory automatically updates across your website with photos, pricing, and details.", icon: Zap },
                        ].map((item) => (
                            <li key={item.title}>
            ...
            <div className="mx-auto -mt-16 flex w-full max-w-container justify-center px-4 pb-16 md:-mt-24 md:px-8 md:pb-24">
                <img src={webLandingImage.src} alt="Website Builder" className="h-60 w-full rounded-lg object-cover md:h-100 lg:h-129" />
            </div>
        </section>
    );
};
        ...
        <section className="bg-secondary py-16 md:py-24">
            <div className="mx-auto max-w-container px-4 md:px-8">
                <figure className="flex flex-col gap-10 lg:flex-row lg:gap-32">
                    <img src={sgOwnerImage.src} alt="Salvador Guerra" className="size-[200px] rounded-2xl object-cover md:size-[328px]" />
                ...
                <div className="grid grid-cols-1 gap-16 lg:grid-cols-2">
                    ...
                    <div className="flex items-center justify-center">
                        <div className="w-full md:max-w-120">
                            ...
                            <Form onSubmit={(e) => { e.preventDefault(); const data = Object.fromEntries(new FormData(e.currentTarget)); console.log("Form data:", data); }} className="mt-12 flex flex-col gap-8">
                                <div className="flex flex-col gap-6">
                                    ...
                                    <InputGroup size="md" name="phone" label="Phone number" leadingAddon={<NativeSelect aria-label="Country code" value={selectedCountryPhone} onChange={(value) => setSelectedCountryPhone(value.currentTarget.value)} options={phoneCodeOptions.map((item) => ({ label: item.label as string, value: item.id as string }))} />}>
                                        <InputBase type="tel" placeholder={countries.find((country) => country.code === selectedCountryPhone)?.phoneMask?.replaceAll("#", "0")} />
                                    </InputGroup>
                                    <TextArea name="message" label="Tell us about your needs" placeholder="What features are most important to you?" rows={4} />
                                    <Checkbox name="privacy" size="md" hint={<>You agree to our friendly{" "}<a href="#" className="rounded-xs underline underline-offset-3 outline-focus-ring focus-visible:outline-2 focus-visible:outline-offset-2">privacy policy.</a></>} />
                                </div>
                                <Button type="submit" size="xl">Request Demo</Button>
                            </Form>
                        </div>
                    </div>
                </div>
