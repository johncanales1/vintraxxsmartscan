 ...
 * Google login button.
 *
 * Uses the Google Identity Services credential flow (`<GoogleLogin />`)
 * which returns a signed `id_token` (JWT). That is what the backend's
 * `POST /auth/google` route expects (it verifies the JWT against Google's
 * tokeninfo endpoint). The previous `useGoogleLogin({ flow: 'implicit' })`
 * returned an OAuth `access_token`, which the backend can NOT verify, so
 * sign-in always failed with 401.
 */
const GoogleLoginButton = ({ onError, setLoading, onSuccess }: {
    ...
    const handleCredentialResponse = async (cred: CredentialResponse) => {
        if (!cred.credential) {
...
export const LoginSimple = ({ resetToken }: { resetToken: string | null }) => {
    ...
    const handleQrCodeSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        // Clear any prior validation banner so a fresh selection starts clean.
        setError("");
        const file = e.target.files?.[0];
        if (file) {
            if (!file.type.startsWith('image/')) {
